...
// Import Stake Engine RGS Client
// Note: Install with: npm install stake-engine
let RGSClient: any;
try {
  // @ts-ignore - Dynamic import for optional dependency
  RGSClient = require('stake-engine').RGSClient;
} catch (error) {
  // Stake Engine not installed - will throw error when trying to use
  RGSClient = null;
}
import type {
  AuthenticateRequest,
...
function initializeClient() {
  ...
  // Check if RGSClient is available
  if (!RGSClient) {
    throw new StakeEngineError(
      'Stake Engine client not available. Install stake-engine package: npm install stake-engine',
      ERROR_CODES.ERR_GSU,
      { hint: 'Run: npm install stake-engine' }
    );
  }
  ...
  try {
    rgsClient = RGSClient({
      url: window.location.href, // Automatically parses sessionID and other params
    });
    
    if (config.logRgsCalls) {
      console.log('[StakeRGS] Client initialized successfully');
    }
  } catch (error) {
    console.error('[StakeRGS] Failed to initialize client:', error);
    throw new StakeEngineError(
      'Failed to initialize Stake Engine client',
      ERROR_CODES.ERR_GSU,
      { originalError: error }
    );
  }
...
export async function authenticate(
  ...
  return wrapApiCall('authenticate', async () => {
    ...
    // Call Stake Engine Authenticate
    const response = await client.Authenticate();
    
    if (config.logRgsCalls) {
      console.log('[StakeRGS] Response:', response);
      console.log('[StakeRGS] Balance:', response.balance.amount);
      console.log('[StakeRGS] Bet levels:', response.config.betLevels);
    }
    ...
    return {
      ...
      config: {
        ...
        jurisdiction: response.config.jurisdiction,
      },
...
export async function getBalance(): Promise<BalanceResponse> {
  return wrapApiCall('getBalance', async () => {
    ...
    if (config.logRgsCalls) {
      console.log('[StakeRGS] getBalance() called');
    }
    
    // Call Stake Engine GetBalance
...
export function displayAmount(
  ...
  try {
    ...
    // Use Stake Engine's DisplayAmount if available
    if (client && client.DisplayAmount) {
    ...
    if (config.enableDebug) {
      console.warn('[StakeRGS] DisplayAmount not available, using fallback');
    }
  }
  
  // Fallback formatting (works without Stake Engine client)
  return `$${(amount / 1000000).toFixed(options?.decimals ?? 2)}`;
}
